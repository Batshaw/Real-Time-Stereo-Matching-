/*
  ...
  This version of the image difference computation app uses plain 1D buffers as internal cl objects 
  (compare to compute_image_differences_using_image_2d.cpp and 
              kernels/simple_example_kernels/compute_image_differences_using_image_2d.cl)
*/
...
void prepare_and_run_compute_difference_image_kernel(int width, int height);
...
int main(int argc, char** argv) {
    ...
    if ( !vm["image_1"].empty() && !vm["image_2"].empty()) { 
      ...
      //return 0; 
        ...
        std::cout << "Image paths were not specified. Using default images:" << std::endl;
        std::cout << image_1_path << std::endl;
        std::cout << image_2_path << std::endl;
        std::cout << std::endl;
    ...
    int width = input_image_1.cols;
    int height = input_image_1.rows;
    ...
    std::string const kernel_file_path = "./kernels/simple_example_kernels/image_difference_kernel_using_buffers.cl";
    std::string const& main_kernel_name = "compute_difference_image";
    ...
    do {
        prepare_and_run_compute_difference_image_kernel(width, height);

        cv::Mat concatenated_result;
        // put both input images and the output image next to each other and display them 
        cv::hconcat(input_image_1, input_image_1, concatenated_result);
...
void prepare_and_run_compute_difference_image_kernel(int width, int height) {
    ...
    clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&cl_input_buffer_1); // set input buffer 1
    clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&cl_input_buffer_2); // set input buffer 2
    clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *)&cl_output_buffer);  // set output buffer
    clSetKernelArg(kernel, 3, sizeof(int), (void *) &width); /* add width of image as kernel argument 
                                                                (is automatically copied from CPU to GPU, because it is a built-in data type)*/
    clSetKernelArg(kernel, 4, sizeof(int), (void *) &height); /* add height of image as kernel argument 
                                                                (is automatically copied from CPU to GPU, because it is a built-in data type)*/
...
void cleanup_cl_resources() {
    ...
    status_code = clReleaseMemObject(cl_input_buffer_1);
    if(CL_SUCCESS != status_code) {
      std::cout << "Failed to release GPU input buffer 1\n";
    }  //Release mem object.
    
    status_code = clReleaseMemObject(cl_input_buffer_2);
    if(CL_SUCCESS != status_code) {
