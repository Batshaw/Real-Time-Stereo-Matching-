...
int main(int argc, char** argv) {
    ...
    std::string const kernel_file_path = "./kernels/simple_example_kernels/window_based_image_binarization.cl";
    std::string const& main_kernel_name = "binarize_image";
    ...
    cv::createTrackbar("Half Window Size", window_name, &g_window_half_size, 120, NULL);
...
void prepare_and_run_window_based_image_binarization_kernel(int width, int height) {
    ...
    clSetKernelArg(kernel, 3, sizeof(int), (void*) &height); /* add height of image as kernel argument 
                                                                (is automatically copied from CPU to GPU, because it is a built-in data type)*/
    clSetKernelArg(kernel, 4, sizeof(int), (void*) &g_window_half_size); //half of window size for sampling the input image
