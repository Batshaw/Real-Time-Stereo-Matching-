#ifndef DSM_GPU_ADAP_WEIGHT_STEREO_MATCHER_H
#define DSM_GPU_ADAP_WEIGHT_STEREO_MATCHER_H
...
class GPUAdaptiveWeightStereoMatcher : public GPUImageProcessorBinary {
public:


	//factory function
    static std::shared_ptr<GPUAdaptiveWeightStereoMatcher> create(cl_context const& context, cl_device_id const& device_id,
                                                   cv::Vec2i const& image_dimensions, ConstWinStereoMatchingMode const& similarity_measure_mode);
	
    GPUAdaptiveWeightStereoMatcher(cl_context const& context, cl_device_id const& device_id, cv::Vec2i const& image_dims, ConstWinStereoMatchingMode const& similarity_measure_mode);
    ~GPUAdaptiveWeightStereoMatcher() {};
...
private:
    ...
    cl_mem cl_ref_weight_buffer_;
    cl_mem cl_search_weight_buffer_;
    float gamma_proximity;
    float gamma_similarity;

    cl_kernel cl_cost_kernel_;
    std::string cl_cost_kernel_name_;

    cl_kernel cl_weight_kernel_;
    std::string cl_weight_kernel_name_;
