...
int main(int argc, char** argv) {
   ...
   initialize_cl_buffers(width, height, input_image.data);
    ...
    std::string const kernel_file_path = "./kernels/simple_task_kernels/sobel_filter.cl";
    std::string const& main_kernel_name = "sobel_filter";
...
void cleanup_cl_resources() {
   ...
   /* status_code = clReleaseMemObject(cl_output_buffer_y);
    if(CL_SUCCESS != status_code) {
      std::cout << "Failed to release GPU input buffer 1\n";
    }  //Release mem object.

    status_code = clReleaseMemObject(cl_output_buffer_x);
    if(CL_SUCCESS != status_code) {
      std::cout << "Failed to release GPU input buffer 1\n";
    }  //Release mem object.
*/
