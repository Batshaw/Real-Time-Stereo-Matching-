...
void _add_pixels_3x8(int2 pixel_2d_index,  
...
/*** main kernels ***/
__kernel void add_images_3x8(__global unsigned char* input_image_1,
	...
	for(pixel_2d_index.y = thread_2d_index.y; pixel_2d_index.y < image_height; pixel_2d_index.y += thread_2d_sizes.y) {
		for(pixel_2d_index.x = thread_2d_index.x; pixel_2d_index.x < image_width; pixel_2d_index.x += thread_2d_sizes.x) {
			_add_pixels_3x8(pixel_2d_index, input_image_1, input_image_2, output_image, image_width, image_height);
		}
	}
}
...
void _add_pixels_3x8(int2 pixel_2d_index,  
	...
	int3 max_value_per_channel = {255, 255, 255};

	int3 addition_result = min(max_value_per_channel, pixel_color_1 + pixel_color_2);

	output_image[pixel_1d_offset + 0] = addition_result.x;
	output_image[pixel_1d_offset + 1] = addition_result.y;
	output_image[pixel_1d_offset + 2] = addition_result.z;	
}